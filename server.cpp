    ...
    while (1) {
        ...
        if (strncmp(buffer, "register ", 9) == 0) {
            ...
            if (verify_login(login_username, login_password)) {
                ...
                send(client_socket, "ERROR:  û            ", 26, 0);
            }
        } else if (strlen(username) == 0) {
            send(client_socket, "ERROR:    ȵ ¼", 18, 0);
            continue;
            ...
            if (!is_valid_username(target_user)) {
                send(client_socket, "ERROR:  û     Ч", 20, 0);
                continue;
            }
            char user_path[512];
            ...
            if (strlen(user) == 0 || strlen(filename) == 0) {
                send(client_socket, "ERROR:      ʽ    ", 22, 0);
                continue;
            }
            if (strcmp(user, username) != 0) {
                send(client_socket, "ERROR: ֻ ܲ    Լ    ļ ", 28, 0);
                continue;
            }
            char path[512];
            sprintf(path, "%s/%s", SERVER_FILES_DIR, username);
            ...
            if (recv_result != 8) {
                send(client_socket, "ERROR:      ļ   Сʧ  ", 28, 0);
                continue;
            }
            ...
            if (!fp) {
                send(client_socket, "ERROR:  ޷      ļ ", 22, 0);
                continue;
            }
            ...
            if (success && received == file_size) {
                ...
                if (_stat(full_path, &st) == 0) {
                    ...
                    if (actual_size != file_size) {
                        send(client_socket, "ERROR:  ļ   С  ƥ  ", 24, 0);
                        remove(full_path);
                    } else {
                        printf("[%s]  ļ     ɹ : %s,   С: %s\n", 
                               username, filename, get_file_size_string(file_size));
                    ...
                    send(client_socket, "ERROR:  ļ   ֤ʧ  ", 22, 0);
                }
            } else {
                send(client_socket, "ERROR:  ļ    䲻    ", 24, 0);
                remove(full_path);
            }
            ...
            if (strcmp(user, username) != 0) {
                send(client_socket, "ERROR: ֻ       Լ    ļ ", 28, 0);
                continue;
            }
            char full_path[512];
            ...
            struct _stat st;
            if (_stat(full_path, &st) != 0) {
                send(client_socket, "ERROR:  ļ       ", 20, 0);
                continue;
            }
            ...
            char response[256];
            sprintf(response, "  ǰ û : %s", username);
            send(client_socket, response, strlen(response), 0);
        } else if (strncmp(buffer, "logout", 6) == 0) {
            send(client_socket, "ע   ɹ ", 12, 0);
            break;
            ...
            char help_msg[] = 
                "        :\n"
                "  register username email password  - ע   û \n"
                "  login username password          -   ¼\n"
                "  save username filename           -  ϴ  ļ  ( û  ռ     : 1GB)\n"
                "  load username filename           -      ļ \n"
                "  list username                    -  鿴 ļ  б Ϳռ ʹ     \n"
                "  space username                   -  鿴 û  ռ ʹ     \n"
                "  whoami                           -   ʾ  ǰ  ¼ û \n"
                "  logout                           - ע    ¼\n"
                "  exit                             -  ˳ \n"
                "  help                             -   ʾ    ";
            ...
            send(client_socket, "ERROR: δ֪    ", 18, 0);
        }
    }
